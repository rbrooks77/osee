...
public class WorkItemJsonWriter implements MessageBodyWriter<IAtsWorkItem> {
   ...
   public void writeTo(IAtsWorkItem config, Class<?> type, Type genericType, Annotation[] annotations, MediaType mediaType, MultivaluedMap<String, Object> httpHeaders, OutputStream entityStream) throws IOException, WebApplicationException {
      ...
      try {
         ...
         addWorkItem(atsApi, orcsApi, config, annotations, writer, getAttributeTypes(), Collections.emptyList());
      } finally {
         if (writer != null) {
            writer.flush();
         }
   ...
   protected static void addWorkItem(AtsApi atsApi, OrcsApi orcsApi, IAtsWorkItem workItem, Annotation[] annotations, JsonGenerator writer, AttributeTypes attributeTypes, List<WorkItemWriterOptions> options) throws JsonGenerationException, IOException {
   ...
   protected static void addWorkItemWithIds(AtsApi atsApi, OrcsApi orcsApi, IAtsWorkItem workItem, JsonGenerator writer, List<WorkItemWriterOptions> options) throws IOException, JsonGenerationException, JsonProcessingException {
      ArtifactReadable workItemArt = (ArtifactReadable) workItem.getStoreObject();
      writer.writeStartObject();
      writer.writeNumberField("id", workItem.getId());
      String atsId = workItemArt.getSoleAttributeValue(AtsAttributeTypes.AtsId, "");
